...
import menschaergerdichnicht.model.{GameField, Move, Token}
import menschaergerdichnicht.fileIO.FileIO
import menschaergerdichnicht.util.UndoManager
...
class DefaultController(using fileIO: FileIO) extends Controller:
  ...
  def possibleMoves: Try[List[Move]] = Try {
    if (gameField.gameState.shouldDice) {
      ...
      menschaergerdichnicht.util.commonFunctions.possibleMoves(gameField)
    }
  }
